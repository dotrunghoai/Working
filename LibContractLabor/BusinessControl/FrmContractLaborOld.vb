...
Public Class FrmContractLaborOld : Inherits DevExpress.XtraEditors.XtraForm
        ...
        If Section Then
            For r As Integer = 0 To BandedGridView1.RowCount - 1
                If BandedGridView1.GetRowCellValue(r, "ChatLuongCV") <= 0 Then
                    ShowWarning(String.Format("MSNV: {0} chưa đánh giá chất lượng công việc !", BandedGridView1.GetRowCellValue(r, "EmpID")))
                    Return False
                End If
                If BandedGridView1.GetRowCellValue(r, "KienThucCV") <= 0 Then
                    ShowWarning(String.Format("MSNV: {0} chưa đánh giá kiến thức công việc !", BandedGridView1.GetRowCellValue(r, "EmpID")))
                    Return False
                End If
                If BandedGridView1.GetRowCellValue(r, "HanhKiem") <= 0 Then
                    ShowWarning(String.Format("MSNV: {0} chưa đánh giá hạnh kiểm !", BandedGridView1.GetRowCellValue(r, "EmpID")))
                    Return False
                End If
                If BandedGridView1.GetRowCellValue(r, "QuanHeDongNghiep") <= 0 Then
                    ShowWarning(String.Format("MSNV: {0} chưa đánh giá quan hệ với đồng nghiệp !", BandedGridView1.GetRowCellValue(r, "EmpID")))
                    Return False
                End If
                If BandedGridView1.GetRowCellValue(r, "Observation") <> "Operator" Then
                    If BandedGridView1.GetFocusedRowCellValue("SangKien") <= 0 Then
                        ShowWarning(String.Format("MSNV: {0} chưa đánh giá sáng kiến !", BandedGridView1.GetRowCellValue(r, "EmpID")))
                        Return False
                    End If
                End If
                If BandedGridView1.GetRowCellValue(r, "Observation") <> "Operator" Then
                    If BandedGridView1.GetFocusedRowCellValue("LapKeHoach") <= 0 Then
                        ShowWarning(String.Format("MSNV: {0} chưa đánh giá khả năng lập kế hoạch, tổ chức !", BandedGridView1.GetRowCellValue(r, "EmpID")))
                        Return False
                    End If
                End If
                If BandedGridView1.GetRowCellValue(r, "Observation") <> "Operator" Then
                    If BandedGridView1.GetFocusedRowCellValue("LanhDao") <= 0 Then
                        ShowWarning(String.Format("MSNV: {0} chưa đánh giá khả năng giám sát và lãnh đạo !", BandedGridView1.GetRowCellValue(r, "EmpID")))
                        Return False
                    End If
                End If
        ...
        If BandedGridView1.Editable And e.Column.ReadOnly = False Then
            ...
            If e.Column.FieldName = "ChatLuongCV" Or e.Column.FieldName = "KienThucCV" Or
                ...
                If BandedGridView1.GetFocusedRowCellValue("Observation") = "Operator" Then
                    Dim sqlScalar As String = String.Format("SELECT SUM(ChatLuongCV + KienThucCV + HanhKiem + QuanHeDongNghiep) * 1.0 / 4
                                                            FROM GA_CT_ReviewContract_Detail
                                                            WHERE ID = '{0}'
                                                            AND	 EmpID = '{1}'",
                                                            txtID.Text,
                    ...
                    End If
                Else
                    Dim sqlScalar As String = String.Format("SELECT SUM(ChatLuongCV + KienThucCV + HanhKiem + QuanHeDongNghiep + SangKien + LapKeHoach + LanhDao) * 1.0 / 7
                                                            FROM GA_CT_ReviewContract_Detail
                                                            WHERE ID = '{0}'
                                                            AND	 EmpID = '{1}'",
                                                            txtID.Text,
