
 Public Class t_CompleteSheetIssueHistory_99
     Public FactoryCode_K As String 
     Public TransactionNumber_K As String 
     Public OrderNumber As String 
     Public OrderProductCode As String 
     Public ItemNumberCode As String 
     Public ItemNumberName As String 
     Public CustomerName As String 
     Public StandardCode As String 
     Public StandardName As String 
     Public Quantity As  Integer 
     Public SalesDueDate As DateTime 
     Public AnswerDate As DateTime 
     Public BoxCount As  Integer 
     Public PackingSize As  Integer 
     Public CoolFlag As String 
     Public InspectionRecordFlag As String 
     Public OperatorCode As String 
     Public TypeFlag As String 
     Public StatusFlag As String 
     Public InputDate As DateTime 
     Public IssueDate As DateTime 
     Public CompleteDate As DateTime 
     Public ChangeDate As DateTime 
 End  Class
