
 Public Class DCS_KQ_Original
     Public Ngay As DateTime 
     Public Customer As String 
     Public Section_K As String 
     Public PdCode_K As String 
     Public Item_K As String 
     Public LotNo_K As String 
     Public EntryDate As DateTime 
     Public Test As String 
     Public ChartType As String 
     Public ActionPlan As String 
     Public DueDate As DateTime 
     Public PIC As String 
     Public Result As String 
     Public RootCause As String 
     Public TemAction As String 
     Public PermanAction As String 
     Public DateImplement As DateTime 
     Public Effective As String 
     Public FinalDecision As String 
     Public Change4M As String 
     Public ClosedDate As DateTime 
 End  Class
