...
Public Class FrmOutTrayAndU00PR : Inherits DevExpress.XtraEditors.XtraForm
    ...
    Private Sub mnuShowAll_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuShowAll.Click
        Try
            ...
            dtAll = _db.ExecuteStoreProcedureTB("[sp_PCM_OutTrayAndU00_PR]", param)
            bd.DataSource = dtAll

            Dim dtCombo As DataTable
            Dim rowCombo As DataRow

            Dim JCode As DataGridViewComboBoxColumn = DirectCast(gridD.Columns("JVName"), DataGridViewComboBoxColumn)
            JCode.DataPropertyName = "JVName"
            JCode.ValueMember = "JVName"
            JCode.DisplayMember = "JVName"
            Dim sqlJCode As String = String.Format("Select distinct JVName " +
            "from {0} Where ('{2}'='' or ECode = '{2}' )" +
            "union select distinct JVName from {1} " +
            "Where  ('{2}'='' or ECode = '{2}' ) order by JVName",
            PublicTable.Table_PCM_MterTrayU00,
            PublicTable.Table_PCM_TrayU00, condiUser)
            dtCombo = _db.FillDataTable(sqlJCode)
            ...
            "Where ('{2}'='' or ECode = '{2}' )",
            PublicTable.Table_PCM_MterTrayU00, PublicTable.Table_PCM_TrayU00, condiUser)
            dtCombo = _db.FillDataTable(sqlSubPrcName)
            rowCombo = dtCombo.NewRow
            dtCombo.Rows.InsertAt(rowCombo, 0)
            ...
            Dim sqlPdCode As String = String.Format("Select distinct PdCode from {0} " +
            "Where ('{2}'='' or ECode = '{2}' )" +
            "union select distinct PdCode from {1} " +
            "Where ('{2}'='' or ECode = '{2}' ) order by PdCode ",
            PublicTable.Table_PCM_MterTrayU00, PublicTable.Table_PCM_TrayU00, condiUser)
            dtCombo = _db.FillDataTable(sqlPdCode)
            rowCombo = dtCombo.NewRow
            dtCombo.Rows.InsertAt(rowCombo, 0)
            ...
            Dim sqlFilter As String = String.Format("Select distinct JVName from {0} Where YMD = '{1}' and ('{2}'='' or ECode = '{2}' )",
            ...
            cboJCodeFilter.DisplayMember = "JVName"
            cboJCodeFilter.ValueMember = "JVName"

            cboJCodeFilter.SelectedValue = DBNull.Value

            sqlFilter = String.Format("Select distinct SubPrcName from {0} Where YMD = '{1}' and ('{2}'='' or ECode = '{2}' )",
                                      PublicTable.Table_PCM_TrayU00, day, condiUser)
            dtFilter = _db.FillDataTable(sqlFilter)
            rowFilter = dtFilter.NewRow()
            dtFilter.Rows.InsertAt(rowFilter, 0)
            ...
            sqlFilter = String.Format("Select distinct ECode from {0} Where YMD = '{1}' and ('{2}'='' or ECode = '{2}' )",
                                      PublicTable.Table_PCM_TrayU00, day, condiUser)
            dtFilter = _db.FillDataTable(sqlFilter)
            rowFilter = dtFilter.NewRow()
            dtFilter.Rows.InsertAt(rowFilter, 0)
            ...
            sqlFilter = String.Format("Select distinct PdCode from {0} Where YMD = '{1}' and ('{2}'='' or ECode = '{2}' )",
                                      PublicTable.Table_PCM_TrayU00, day, condiUser)
            dtFilter = _db.FillDataTable(sqlFilter)
            rowFilter = dtFilter.NewRow()
            dtFilter.Rows.InsertAt(rowFilter, 0)
            ...
            If dtAll.Rows.Count <> 0 Then
                tsTotal1.Text = CType(dtAll.Compute("sum(MatQty1)", ""), Decimal).ToString("N2")
                tsTotal2.Text = CType(dtAll.Compute("sum(MatQty2)", ""), Decimal).ToString("N2")
            End If
    ...
    Private Sub gridD_EditingControlShowing(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewEditingControlShowingEventArgs) Handles gridD.EditingControlShowing
        ...
        Try
            ...
            If gridD.CurrentCell.ColumnIndex = gridD.Columns("JVName").Index Then
                ...
                If TypeOf (e.Control) Is DataGridViewComboBoxEditingControl Then
                    ...
                    cboJCode.DisplayMember = "JVName"
                    cboJCode.ValueMember = "JVName"
                    cboJCode.BackColor = Color.White
                    Dim sqlJCode As String = String.Format("select distinct JCode, JEName, JVName " +
                    "from {0} " +
                    "Where ECode = '{1}' and PdCode='{2}' and Old = 'False' and XuatKho='1' ",
                    PublicTable.Table_PCM_MterTrayU00, CurrUser, gridD.CurrentRow.Cells("PdCode").Value)
                    dt = _db.FillDataTable(sqlJCode)
                    'row = dt.NewRow
                    'dt.Rows.InsertAt(row, 0)
                    cboJCode.DataSource = dt
                    cboJCode.SelectedValue = gridD.CurrentRow.Cells("JVName").Value
            ...
            If gridD.CurrentCell.ColumnIndex = gridD.Columns("SubPrcName").Index Then
                ...
                If TypeOf (e.Control) Is DataGridViewComboBoxEditingControl Then
                    ...
                    "from {0} " +
                    "Where ( '{1}'='' or ECode = '{1}') and JCode = '{2}' and Old = 'False'", PublicTable.Table_PCM_MterTrayU00,
                    CurrUser, gridD.CurrentRow.Cells("JCode").Value)
    ...
    Private Sub cboJCode_Validating()

        If IsView Then Exit Sub
		If cboJCode.SelectedValue Is Nothing Then
			Return
		End If

		Dim MyJName As String = IIf(IsDBNull(cboJCode.SelectedValue), "", cboJCode.SelectedValue)
        If gridD.CurrentRow.Cells("ECode").Value Is Nothing Or gridD.CurrentRow.Cells("ECode").Value Is DBNull.Value Then
            gridD.CurrentRow.Cells("ECode").Value = CurrUser()
        End If
        If MyJName.ToUpper = "KHAY" Then
            MyJName = "B00"
        ElseIf MyJName.ToUpper.Contains("THOMSON") Then
            MyJName = "H00"
        Else
            MyJName = _db.ExecuteScalar(String.Format("SELECT TOP 1 Cc
                                                       ...
                                                       gridD.CurrentRow.Cells("PdCode").Value,
                                                       gridD.CurrentRow.Cells("JVName").Value))
        End If
        Dim sqlJCode As String = String.Format(" SELECT  [JCode] " +
                                              "  FROM [PCM_MterTrayU00] " +
                                              "  where   ecode='{0}' and  Pdcode='{1}' and Cc = '{2}' and XuatKho='1'  ",
                                              gridD.CurrentRow.Cells("ECode").Value,
                                              gridD.CurrentRow.Cells("PdCode").Value,
                                              MyJName)
        Dim MyJCode = _db.ExecuteScalar(sqlJCode)
        If MyJCode = "" Then
            Return
        End If
        Dim MSubPrcName As String = gridD.CurrentRow.Cells("SubPrcName").Value.ToString

        Dim MPdCode As String = gridD.CurrentRow.Cells("PdCode").Value.ToString

        Dim MJCodeTemp As String = gridD.CurrentRow.Cells("JCodeTemp").Value.ToString
        Dim MSubPrcNameTemp As String = gridD.CurrentRow.Cells("SubPrcNameTemp").Value.ToString
        Dim MPdCodeTemp As String = gridD.CurrentRow.Cells("PdCodeTemp").Value.ToString

        If MyJCode = MJCodeTemp And MSubPrcName = MSubPrcNameTemp And MPdCode = MPdCodeTemp Then Exit Sub
        ...
        Dim sql As String = String.Format("select JCode, JEName, JVName " +
                                        "from {0} where JCode = '{1}'", PublicTable.Table_PCM_MterTrayU00, MyJCode)
        Dim dt As DataTable = _db.FillDataTable(sql)
        If dt.Rows.Count <> 0 Then
            gridD.CurrentRow.Cells("JEName").Value = dt.Rows(0).Item("JEName")
            gridD.CurrentRow.Cells("JCode").Value = dt.Rows(0).Item("JCode")
        End If


    End Sub

    Private Sub cbobox_Validating()
        If IsView Then Exit Sub

        Dim JCode As String = gridD.CurrentRow.Cells("JCode").Value.ToString
        Dim SubPrcName As String = ""
        If cbobox IsNot Nothing Then
            SubPrcName = cbobox.SelectedValue
        End If

        Dim PdCode As String = gridD.CurrentRow.Cells("PdCode").Value.ToString

        Dim JCodeTemp As String = gridD.CurrentRow.Cells("JCodeTemp").Value.ToString
        Dim SubPrcNameTemp As String = gridD.CurrentRow.Cells("SubPrcNameTemp").Value.ToString
        Dim PdCodeTemp As String = gridD.CurrentRow.Cells("PdCodeTemp").Value.ToString
    ...
    Private Sub cboPdCode_Validating()
        If IsView Then Exit Sub
        Dim MPdCode As String = ""
        Dim MJCode As String = gridD.CurrentRow.Cells("JCode").Value.ToString
        Dim MSubPrcName As String = gridD.CurrentRow.Cells("SubPrcName").Value.ToString
        If gridD.CurrentRow.Cells("PdCode").Value IsNot DBNull.Value Then
            MPdCode = gridD.CurrentRow.Cells("PdCode").Value
        Else
            Return
        End If
        ...
        If MJCode = MJCodeTemp And MSubPrcName = MSubPrcNameTemp And MPdCode = MPdCodeTemp Then Exit Sub
        ...
        For i As Integer = 0 To gridD.Rows.Count - 2
            If gridD.CurrentRow.Index <> gridD.Rows(i).Index AndAlso _
                MJCode = gridD.Rows(i).Cells("JCode").Value.ToString() AndAlso _
                MSubPrcName = gridD.Rows(i).Cells("SubPrcName").Value.ToString() AndAlso _
                MPdCode = gridD.Rows(i).Cells("PdCode").Value.ToString() AndAlso _
        ...
        Dim sql As String = String.Format("select A.ECode, A.JCode, A.PdCode, A.PrcName, A.SubPrcName, A.UnitLot, A.Unit, A.MinQty, B.DeptName " +
                                        ...
                                        "where A.ECode = '{1}' and A.JCode = '{2}' and A.SubPrcName = '{3}' and A.PdCode = '{4}'",
                                        PublicTable.Table_PCM_MterTrayU00, CurrUser, MJCode, MSubPrcName, MPdCode)
        Dim dt As DataTable = _db.FillDataTable(sql)
        If dt.Rows.Count <> 0 Then
            ...
            "Group by JCode, PdCode", PublicTable.Table_PCM_TrayU00, MJCode, MPdCode)
            ...
            obj.JCode_K = MJCodeTemp
            obj.SubPrcName_K = MSubPrcNameTemp
            obj.PdCode_K = MPdCodeTemp
            _db.GetObject(obj)
            ...
            If _db.ExistObject(obj) Then
                ...
                obj.ECode_K = gridD.CurrentRow.Cells("ECode").Value
                obj.JCode_K = MJCode
                obj.SubPrcName_K = MSubPrcName
                obj.PdCode_K = MPdCode
                ...
                _db.Update(obj, String.Format("YMD = '{0}' and ECode = '{1}' and JCode = '{2}' and SubPrcName = '{3}' and PdCode = '{4}'", _
                                               gridD.CurrentRow.Cells("YMD").Value.ToString, _
                                              gridD.CurrentRow.Cells("ECode").Value.ToString, _
                                              MJCodeTemp, MSubPrcNameTemp, MPdCodeTemp))
                gridD.CurrentRow.Cells("JCodeTemp").Value = MJCode
                gridD.CurrentRow.Cells("SubPrcNameTemp").Value = MSubPrcName
                gridD.CurrentRow.Cells("PdCodeTemp").Value = MPdCode
            Else
                obj.YMD_K = gridD.CurrentRow.Cells("YMD").Value
                obj.ECode_K = gridD.CurrentRow.Cells("ECode").Value
                obj.JCode_K = MJCode
                ...
                gridD.CurrentRow.Cells("PdCode").Value = MPdCode
            End If
    ...
    Function sumOut(ByVal YMD As String, ByVal ECode As String, ByVal PdCode As String, ByVal JCode As String, ByVal SubPrcName As String) As Decimal
        ...
        "select MatQty1, MatQty2, AdjustQty from #T  T where T.ID NOT IN ('{3}')", _
    ...
    Sub LockColums(ByVal lock As Boolean)
        gridD.Columns("JVName").ReadOnly = Not lock
    ...
    Private Sub cboJCodeFilter_SelectedIndexChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboJCodeFilter.SelectedIndexChanged
        ...
        Else
            ...
            dv.RowFilter = "[JVName] LIKE '%" + cboJCodeFilter.SelectedValue + "%'"
            Dim bd As New BindingSource()
            bd.DataSource = dv
            gridD.DataSource = bd
            bnGrid.BindingSource = bd
    ...
    Private Sub gridD_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles gridD.Click
        If gridD.CurrentRow Is Nothing Then Exit Sub
        Dim JCode As String = gridD.CurrentRow.Cells("JCode").Value.ToString
        tsStock.Text = stock(JCode).ToString("N2")
        tsInsStockD.Text = avalQty.ToString("N2")
        tsNonInsStockD.Text = heldQty.ToString("N2")
    End Sub
    ...
    '        If _db.ExistObject(objUpdate) Then
    '            _db.GetObject(objUpdate)
    ...
    Private Sub mnuEdit_Click(sender As System.Object, e As System.EventArgs) Handles mnuEdit.Click
        If cls.checkUser Then
            ...
            gridD.Columns("JVname").ReadOnly = False
            ...
            'gridD.Columns("XuatKho").ReadOnly = False
        End If
    End Sub

    Private Sub mnuExportSum_Click(sender As System.Object, e As System.EventArgs) Handles mnuExportSum.Click
        Dim sql As String = String.Format("select YMD, DeptName, JCode, PrcName, JEName, JVName, Unit, Sum(MatQty1) MatQty1, Sum(MatQty2) MatQty2, Note = ISNULL(Note,'') " +
        "from {0} " +
        "where YMD = '{1}' and ECode like '%{2}%'" +
        "group by YMD, DeptName, JCode, PrcName, JEName, JVName, Unit, ISNULL(Note,'') " +
        "order by DeptName, PrcName, JCode", PublicTable.Table_PCM_TrayU00, dtpOrderDate.Value.ToString("yyyyMMdd"), condiUser)
        ExportEXCEL(_db.FillDataTable(sql))
    End Sub
    ...
    Private Sub gridD_CellValueChanged(sender As System.Object, e As System.Windows.Forms.DataGridViewCellEventArgs) Handles gridD.CellValueChanged
        ...
        Try
            ...
            If e.ColumnIndex = gridD.Columns("JVName").Index _
            ...
            If e.ColumnIndex = gridD.Columns("LotE1").Index Then
                ...
                If _LotE1 < _LotS1 And _LotE1 <> 0 Then
                    MessageBox.Show("Input LotE1 >= LotS1", "Input LotE1")
                    Exit Sub
                End If

                If _LotE1 >= _LotS2 And _LotS2 <> 0 Then
                    MessageBox.Show("Input LotE1 < LotS2", "Input LotE1")
                    Exit Sub
                End If
            ...
            If e.ColumnIndex = gridD.Columns("AdjustQty").Index Then
                ...
                'Dim dtmaxday As DataTable = _db.FillDataTable(sqlmaxday)
