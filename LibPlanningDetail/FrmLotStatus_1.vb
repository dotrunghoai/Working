...
Public Class FrmLotStatus_1 : Inherits DevExpress.XtraEditors.XtraForm
...
#Region "User Function"
    ...
    Private Function ExistPeriodCode() As Boolean
        ...
        Dim tbl As DataTable = DB.FillDataTable(String.Format("SELECT * FROM PD_LotStatusHead_1 WHERE PeriodCode='{0}'", sPeriodCode))
    ...
    Private Sub LoadAll()
        ...
        Dim tbl As DataTable = DB.ExecuteStoreProcedureTB("sp_PD_LotStatusHead_1", para)
    ...
    Private Sub LoadDataCombo()
        Dim tbl As DataTable = DB.FillDataTable(" SELECT PeriodCode FROM dbo.PD_LotStatusHead_1 GROUP BY PeriodCode " +
    ...
    Private Sub CreateDailyOutput(ByVal tbl As DataTable)
        ...
        Dim tblDetail As DataTable = DB.ExecuteStoreProcedureTB("sp_PD_LotStatusHead_1", para)
        ...
        For Each r As DataRow In tbl.Rows
            ...
            For Each c As DataColumn In tblOutput.Columns
                ...
                Dim sProductCode As String = CType(r("ProductCode"), String)
                Dim SStartLot As String = CType(r("StartLot"), String)
                ...
                If rs.Length > 0 Then
                    rAdd(c.ColumnName) = If(CType(rs(0)("Quantity"), Int32) = 0, DBNull.Value, rs(0)("Quantity"))
                End If
            Next
...
#Region "Form Function"
    ...
    Private Sub mnuImport_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles mnuImport.Click
        Try
            ...
            DB.ExecuteStoreProcedure("sp_PD_LotStatusHead_1", para)
            ...
            For Each r As DataRow In tbl.Rows
                ...
                Dim obj As New PD_LotStatusHead_1
            ...
            For Each r As DataRow In tbl.Rows
                ...
                For i As Int32 = 2 To tbl.Columns.Count - 1
                    Dim obj As New PD_LotStatusDetail_1
                    obj.ProductCode_K = sProductCode
                    obj.StartLot_K = If(r("StartLot") Is DBNull.Value, String.Empty, r("StartLot"))
...
#End Region
    ...
    Private Sub cboPeriod_SelectedValueChanged(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cboPeriod.SelectedValueChanged
        Try
            Dim tbl As DataTable = DB.FillDataTable(String.Format("SELECT TOP 1 FromDate, ToDate FROM dbo.PD_LotStatusHead_1 WHERE PeriodCode = '{0}'", cboPeriod.Text))
