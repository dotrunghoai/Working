...
#Region "User Function"
    ...
    Private Sub LoadComboBox()
        bCboLoading = True
        Dim sql As String = String.Format("SELECT CustomerCode, CustomerName FROM PD_MsCustomer")
    ...
    Private Sub LoadLeadTime()
        ...
        Dim sql As String = String.Format("SELECT ID,LeadTimeName,CustomerCode 
                                            FROM PD_MsLeadTime 
    ...
    Private Sub CreatePlanningDetail()
        ...
        For Each rCus As DataRow In tb.Rows
            ...
            For Each item In lstByCustomer
                ...
                Dim tblPrcErp As DataTable = DB.FillDataTable(
                    String.Format("SELECT ProcessCode, ProcessNumber = ISNULL(ProcessNumber,''), ProcessName AS ProcessNameE 
                        FROM PD_ProcessLeadTime 
                        WHERE LeadTimeID = '{0}' 
                        ORDER BY Idx",
                        item("LeadTimeID")))
                ...
                lstPdLeadTime = DB.FillDataTable(
                    String.Format("SELECT * 
                                    FROM PD_ProcessLeadTime 
                                    WHERE LeadTimeID = '{0}' 
                                    ORDER BY Idx",
                                    item("LeadTimeID"))).AsEnumerable().ToList()
                If lstPdLeadTime.Count = 0 Then Continue For
...
#Region "Form Function"
    ...
    Private Sub CreatePlanningTotal()
        ...
        Dim tblPdFilm As DataTable = DB.FillDataTable(String.Format("SELECT ProductCode FROM PD_MsPdFilm"))
